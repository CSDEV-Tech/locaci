    ...
    >
      <div className={"relative w-full"}>
        ...
        <label
          htmlFor={id}
          className={clsx(
            "absolute left-0 -top-3.5 text-gray font-thin text-sm transition-all font-semibold",
  ...
  function handleChange(newValue: string) {
    ...
    if (newValue.length === 0 && value !== 0) {
      ...
      if (
        ...
        onChange && onChange(Math.min(max, Math.max(min, parseInt(newValue))));
      }
    }
  }
  ...
  return (
    ...
    >
      <div className={"relative w-full flex items-center gap-1"}>
        ...
        {appendix && (
          <span className={`font-semibold text-gray`}>{appendix}</span>
        )}
